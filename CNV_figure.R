...
# generate figure for shared and group-uniq CNVs 
mydata <- read.csv("./CNV_visul.info_by_group.csv")
mydata <- mydata[grep(paste(c("random", "chrUn", "chrM"),collapse="|"),mydata$chromosome, invert=TRUE),] 
...
pdf("./CNVs_per_chr_Mb_Manta_group.pdf",14,20)
ggplot(mydata, aes(x=New_start, y=group)) + #geom_point() +
  geom_segment(aes(xend=New_end, yend=group, colour=CNV_type)) +  #
  geom_segment(aes(x=New_start, y=group, xend=New_end, yend=group, size=15, colour=CNV_type)) +  #, colour=CNV_type
  #geom_hline(yintercept=2, linetype=2) +
  facet_wrap(~ chromosome, scales = "free", ncol=2) +
  xlab("Position (Mb)") +
  ylab("Chromosome") +
  scale_x_continuous(breaks = round(seq(0, max(mydata$New_end), by = 10),0)) + 
  #scale_x_continuous(limits = c(0,200)) +
  #scale_size_continuous(range = c(4:22000001, 22432001:22467000))+
  theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.background = element_blank(), axis.line = element_line(colour = "black"))
dev.off()
